from PyQt5.QtWidgets import (
    ...
    QHBoxLayout, QVBoxLayout, QPushButton, QFrame, QLabel, QSizePolicy
)
...
class ChatSection(QWidget):
    def __init__(self):
        ...
        gif_name = f"{Assistantname}.gif" if os.path.exists(GraphicsDirectoryPath(f"{Assistantname}.gif")) else "Aurio.gif"
        movie = QMovie(GraphicsDirectoryPath(gif_name))
        max_gif_size_W = 480
        max_gif_size_H = 270
        ...
        # Keep single gif label addition (original repeated add removed)
        font = QFont()
    ...
    def load_icon(self, path, width=60, height=60):
        ...
        # ensure icon_label exists before setting (some callers expect it)
        if hasattr(self, 'icon_label'):
            self.icon_label.setPixmap(new_pixmap)

    def toggle_icon(self, event=None):
        # this method was present but toggled and icon_label are expected externally
...
class InitialScreen(QWidget):
    def __init__(self, parent=None):
        ...
        # compute height similar to original intent
        ...
        # fixed alignment call name
        self.icon_label.setAlignment(Qt.AlignCenter)

        self.toggled = True
        # do not call toggle_icon here to avoid double-initialization issues,
        # keep original behavior by setting mousePressEvent
